...
public class AiWorkflowRuntimeController {
	...
	public SseEmitter run(@RequestBody WorkflowRunDTO workflowRunDTO) throws GraphStateException, InterruptedException {
		// workflowStarter.streaming(workflowRunDTO.getWorkflowId(),
		// workflowRunDTO.getInputs(),workflowRunDTO.getConversationId());
		...
		ThreadUtil.execAsync(() -> {
			...
			List<NodeOutputVariable> list = List.of(new NodeOutputVariable("name", "string", "开始节点"));
			map.put("nodeId", "start-node");
			map.put("nodeName", "开始节点");
			map.put("outputs", JacksonUtil.toJsonString(list));
			SSEEmitterHelper.parseAndSendPartialMsg(sseEmitter, JacksonUtil.toJsonString(map));
			...
			for (int i = 0; i < msg.length(); i++) {
				...
				SSEEmitterHelper.parseAndSendPartialMsg(sseEmitter, JacksonUtil.toJsonString(tmap));
				try {
