          ...
          <view v-for="(order, index) in tabItem.orderList" :key="index" class="order-item">
            ...
            <view v-if="order.orderItems.length === 1" class="goods-box-single"
              ...
              <view class="right">
                ...
                <text class="price">{{ orderItem.price / 100 }}</text>
              </view>
            </view>
            <view class="price-box">
              共<text class="num">{{ order.totalQuantity }}</text>件商品 实付款
              <text class="price">{{ order.payAmount / 100 }}</text>
            </view>
...
import {cancelOrder, deleteOrder, listOrdersWithPage} from "../../api/order/order.js";
    ...
    loadData() {
      ...
      setTimeout(() => {
        ...
        listOrdersWithPage(params).then((response) => {
          console.log('订单列表数据', response.data.records)
          navItem.orderList = response.data.records;
        });
