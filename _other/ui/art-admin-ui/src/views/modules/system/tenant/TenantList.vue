  ...
  <div>
    <div class="m-3 p-3 bg-white">
      ...
      >
        ...
        <vxe-column field="name" title="租户名称" />
        <vxe-column field="tenantAdminName" title="租户管理员" />
        <vxe-column field="status" title="租户状态">
          <template #default="{ row }">
            <span v-if="row.status.toString() === '0'"> 正常 </span>
            <span v-else> 异常 </span>
          </template>
        </vxe-column>
        <vxe-column field="expireTime" title="过期时间" width="200" />
        <vxe-column field="accountCount" title="账号数量" width="100" />
      ...
      <!--编辑表单-->
      <TenantEdit ref="tenantEdit" @ok="queryPage" />
    </div>
  </div>
</template>
...
<script lang="ts" setup>
  ...
  import { page, del } from '/@/api/system/tenant/index'
   ...
   */
  function add() {
    tenantEdit.value?.init(null, FormOperationType.ADD)
  }
   ...
   */
  function show(item) {
    tenantEdit.value?.init(item.id, FormOperationType.SHOW)
  }
   ...
   */
  function edit(item) {
    tenantEdit.value?.init(item.id, FormOperationType.EDIT)
  }
