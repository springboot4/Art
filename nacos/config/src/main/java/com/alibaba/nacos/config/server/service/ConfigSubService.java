...
public class ConfigSubService {
	...
	public ConfigSubService(ServerListService serverListService1) {
		...
		scheduler = Executors.newScheduledThreadPool(ThreadUtil.getSuitableThreadCount(), new ThreadFactory() {
			@Override
	...
	private List<SampleResult> runCollectionJob(String url, Map<String, String> params,
		...
		for (String ip : ipList) {
			try {
				...
				LogUtil.defaultLog.warn("Get client info from {} with exception: {} during submit job", ip,
						e.getMessage());
			}
		}
		// 获取结果并合并
			...
			try {
				...
				try {
					if (f != null) {
						...
						LogUtil.defaultLog.warn("The task in ip: {}  did not completed in 1000ms ", ipList.get(i));
					}
				}
