...
public class PersistService {
	...
	static final class ConfigInfoWrapperRowMapper implements RowMapper<ConfigInfoWrapper> {
	...
	static final class ConfigInfo4BetaRowMapper implements RowMapper<ConfigInfo4Beta> {
	...
	static final class ConfigInfoBaseRowMapper implements RowMapper<ConfigInfoBase> {
	...
	public boolean addAggrConfigInfo(final String dataId, final String group, String tenant, final String datumId,
		...
		String select = "SELECT content FROM config_info_aggr WHERE data_id = ? AND group_id = ? AND tenant_id = ?  AND "
				+ "datum_id = ?";
		String insert = "INSERT INTO config_info_aggr(data_id, group_id, tenant_id, datum_id, app_name, content, gmt_modified) "
				+ "VALUES(?,?,?,?,?,?,?) ";
		String update = "UPDATE config_info_aggr SET content = ? , gmt_modified = ? WHERE data_id = ? AND group_id = ? AND "
			...
			Boolean isReplaceOk = tjt.execute(new TransactionCallback<Boolean>() {
				...
				public Boolean doInTransaction(TransactionStatus status) {
					try {
						...
						String tenantTmp = StringUtils.isBlank(tenant) ? StringUtils.EMPTY : tenant;
						String sql = "INSERT INTO config_info_aggr(data_id, group_id, tenant_id, datum_id, app_name, "
	...
	public Page<ConfigInfo> findAllConfigInfo(final int pageNo, final int pageSize, final String tenant) {
		...
		String sqlFetchRows = " SELECT t.id,data_id,group_id,tenant_id,app_name,content,md5 "
				+ " FROM (                               " + "   SELECT id FROM config_info         "
				+ "   WHERE tenant_id like ?                  " + "   ORDER BY id LIMIT ?,?             "
	...
	public Page<ConfigKey> findAllConfigKey(final int pageNo, final int pageSize, final String tenant) {
		...
		String select = " SELECT data_id,group_id,app_name " + " FROM (                               "
				...
				+ "   ORDER BY id LIMIT ?, ?             " + " ) g, config_info t                   "
				+ " WHERE g.id = t.id                    ";
	...
	public Page<ConfigInfoBase> findAllConfigInfoBase(final int pageNo, final int pageSize) {
		String sqlCountRows = "SELECT COUNT(*) FROM config_info";
		String sqlFetchRows = " SELECT t.id,data_id,group_id,content,md5 " + " FROM (                               "
				+ "   SELECT id FROM config_info         " + "   ORDER BY id LIMIT ?,?             "
				+ " ) g, config_info t                   " + " WHERE g.id = t.id                    ";

		PaginationHelper<ConfigInfoBase> helper = new PaginationHelper<ConfigInfoBase>();
		try {
			return helper.fetchPageLimit(jt, sqlCountRows, sqlFetchRows,
					new Object[] { (pageNo - 1) * pageSize, pageSize }, pageNo, pageSize, CONFIG_INFO_BASE_ROW_MAPPER);
		}
		catch (CannotGetJdbcConnectionException e) {
			fatalLog.error("[db-error] " + e.toString(), e);
			throw e;
	...
	public Page<ConfigInfoWrapper> findAllConfigInfoFragment(final long lastMaxId, final int pageSize) {
		String select = "SELECT id,data_id,group_id,tenant_id,app_name,content,md5,gmt_modified from config_info where id > ? "
	...
	public Page<ConfigInfoBetaWrapper> findAllConfigInfoBetaForDumpAll(final int pageNo, final int pageSize) {
		...
		String sqlFetchRows = " SELECT t.id,data_id,group_id,tenant_id,app_name,content,md5,gmt_modified,beta_ips "
				...
				+ "   ORDER BY id LIMIT ?,?             " + " ) g, config_info_beta t                   "
				+ " WHERE g.id = t.id                    ";
	...
	public Page<ConfigInfoTagWrapper> findAllConfigInfoTagForDumpAll(final int pageNo, final int pageSize) {
		...
		String sqlFetchRows = " SELECT t.id,data_id,group_id,tenant_id,tag_id,app_name,content,md5,gmt_modified "
				+ " FROM (                               " + "   SELECT id FROM config_info_tag         "
				+ "   ORDER BY id LIMIT ?,?             " + " ) g, config_info_tag t                   "
				+ " WHERE g.id = t.id                    ";
	...
	public List<ConfigInfo> findConfigInfoByBatch(final List<String> dataIds, final String group, final String tenant,
		...
		String sqlStart = "select data_id, group_id, tenant_id, app_name, content from config_info where group_id = ? and "
	...
	public Page<ConfigInfo> findConfigInfoLike4Page(final int pageNo, final int pageSize, final String dataId,
		...
		if (StringUtils.isNotBlank(configTags)) {
			...
			sqlFetchRows = "select a.ID,a.data_id,a.group_id,a.tenant_id,a.app_name,a.content from config_info  a left join "
					+ "config_tags_relation b on a.id=b.id ";
	...
	public ConfigInfoAggr findSingleConfigInfoAggr(String dataId, String group, String tenant, String datumId) {
		String tenantTmp = StringUtils.isBlank(tenant) ? StringUtils.EMPTY : tenant;
		String sql = "SELECT id,data_id,group_id,tenant_id,datum_id,app_name,content FROM config_info_aggr WHERE data_id=? "
	...
	public Page<ConfigInfoAggr> findConfigInfoAggrLike(final int pageNo, final int pageSize, ConfigKey[] configKeys,
		...
		try {
			Page<ConfigInfoAggr> result = helper.fetchPage(jt, sqlCountRows + where, sqlFetchRows + where,
					params.toArray(), pageNo, pageSize, CONFIG_INFO_AGGR_ROW_MAPPER);
			return result;
		}
		catch (CannotGetJdbcConnectionException e) {
			fatalLog.error("[db-error] " + e.toString(), e);
	...
	/**
	 * @author klw
	 * @Description: Delete configuration; database atomic operation, minimum SQL action,
	 * no business encapsulation
	 * @Date 2019/7/5 16:39
	 * @Param [id]
	 * @return void
	 */
	...
	public Page<ConfigHistoryInfo> findConfigHistory(String dataId, String group, String tenant, int pageNo,
		...
		try {
			page = helper.fetchPage(this.jt, sqlCountRows, sqlFetchRows, new Object[] { dataId, group, tenantTmp },
					pageNo, pageSize, HISTORY_LIST_ROW_MAPPER);
		}
		catch (DataAccessException e) {
	...
	private List<ConfigInfoWrapper> listGroupKeyMd5ByPage(int pageNo, int pageSize) {
		...
		PaginationHelper<ConfigInfoWrapper> helper = new PaginationHelper<ConfigInfoWrapper>();
		try {
			Page<ConfigInfoWrapper> page = helper.fetchPageLimit(jt, sqlCountRows, sqlFetchRows,
					new Object[] { (pageNo - 1) * pageSize, pageSize }, pageNo, pageSize,
					CONFIG_INFO_WRAPPER_ROW_MAPPER);
	...
	public Boolean completeMd5() {
		...
		for (int pageNo = 1; pageNo <= pageCount; pageNo++) {
			Page<PersistService.ConfigInfoWrapper> page = findAllConfigInfoForDumpAll(pageNo, perPageSize);
			if (page != null) {
	...
	/**
	 * query all configuration information according to group, appName, tenant (for
	 * export)
	 * @param group
	...
	/**
	 * batch operation,insert or update the format of the returned: succCount: number of
	 * successful imports skipCount: number of import skips (only with skip for the same
	 * configs) failData: import failed data (only with abort for the same configs)
