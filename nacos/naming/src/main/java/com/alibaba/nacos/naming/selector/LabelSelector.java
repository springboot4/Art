 ...
 * <h2>Backgroup</h2> Consider service providers are deployed in two sites i.e. site A and
 * site B, and consumers of this service provider are also deployed in site A and site B.
 * So the consumers may want to visit the service provider in current site, thus consumers
 * in site A visit service providers in site A and consumers in site B visit service
 * providers in site B. This is quite useful to reduce the transfer delay of RPC. This is
 * called same-site-prior strategy.
 * <h2>Same Label Prior</h2> The same-site-prior strategy covers many circumstances in
 * large companies and we can abstract it to a higher level strategy: same-label-prior.
 * <p>
 * So the idea is that presumed we have built a self-defined or integrated a third-party
 * idc CMDB which stores all the labels of all IPs. Then we can filter provider IPs by the
 * consumer IP and we only return the providers who have the same label values with
 * consumer. We can define the labels we want to include in the comparison.
 * <p>
 * If no provider has the same label value with the consumer, we fall back to give all
 * providers to the consumer. Note that this fallback strategy may also be abstracted in
 * future to introduce more kinds of behaviors.
 *
 * @author nkorange
 * @see CmdbReader
 * @since 0.7.0
...
public class LabelSelector extends ExpressionSelector implements Selector {
	...
	public List<Instance> select(String consumer, List<Instance> providers) {
		...
		for (Instance instance : providers) {
			...
			for (String labelName : getLabels()) {
				...
				String consumerLabelValue = getCmdbReader().queryLabel(consumer, PreservedEntityTypes.ip.name(),
						labelName);
	...
	public static class ExpressionInterpreter {
		 ...
		 * <p>
		 * Currently we support the very single type of expression: <pre>
		 *     consumer.labelA = provider.labelA & consumer.labelB = provider.labelB
		 * </pre> Later we will implement a interpreter to parse this expression in a
		 * standard LL parser way.
