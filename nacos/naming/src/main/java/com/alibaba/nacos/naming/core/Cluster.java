...
public class Cluster extends com.alibaba.nacos.api.naming.pojo.Cluster implements Cloneable {
	...
	/**
	 * a addition for same site routing, can group multiple sites into a region, like
	 * Hangzhou, Shanghai, etc.
	 */
	 ...
	 * Create a cluster.
	 * <p>
	 * the cluster name cannot be null, and only the arabic numerals, letters and endashes
	 * are allowed.
	 * @param clusterName the cluster name
	 * @param service the service to which the current cluster belongs
	 * @throws IllegalArgumentException the service is null, or the cluster name is null,
	 * or the cluster name is illegal
	 * @author jifengnan 2019-04-26
	 * @since 1.0.1
	 */
	 ...
	 * <p>
	 * the service shouldn't be replaced. so if the service is not empty will nothing to
	 * do. (the service fields can be changed, but the service A shouldn't be replaced to
	 * service B). If the service of a cluster is required to replace, actually, a new
	 * cluster is required.
	 ...
	 * <p>
	 * Note that the returned service name is not the name which set by
	 * {@link #setServiceName(String)}, but the name of the service to which the current
	 * cluster belongs.
	...
	@Override
	public int hashCode() {
		return new HashCodeBuilder(17, 37).append(getName()).append(service).toHashCode();
	}
	...
	public boolean equals(Object o) {
		...
		return new EqualsBuilder().append(getName(), cluster.getName()).append(service, cluster.service).isEquals();
	}
	...
	public void update(Cluster cluster) {
		...
		if (defCkport != cluster.getDefCkport()) {
			Loggers.SRV_LOG.info("[CLUSTER-UPDATE] {}:{}, defCkport: {} -> {}", getService().getName(), getName(),
					defCkport, cluster.getDefCkport());
			defCkport = cluster.getDefCkport();
		}

		if (defIPPort != cluster.getDefIPPort()) {
			Loggers.SRV_LOG.info("[CLUSTER-UPDATE] {}:{}, defIPPort: {} -> {}", getService().getName(), getName(),
					defIPPort, cluster.getDefIPPort());
			defIPPort = cluster.getDefIPPort();
		}

		if (!StringUtils.equals(sitegroup, cluster.getSitegroup())) {
			Loggers.SRV_LOG.info("[CLUSTER-UPDATE] {}:{}, sitegroup: {} -> {}", getService().getName(), getName(),
