...
public class ServiceManager implements RecordListener<Service> {
	...
	public void onChange(String key, Service service) throws Exception {
		try {
			...
			if (oldDom != null) {
				...
				// re-listen to handle the situation when the underlying listener is
				// removed:
				consistencyService.listen(
						KeyBuilder.buildInstanceListKey(service.getNamespaceId(), service.getName(), true), oldDom);
				consistencyService.listen(
						KeyBuilder.buildInstanceListKey(service.getNamespaceId(), service.getName(), false), oldDom);
			}
			else {
	...
	public int getPagedService(String namespaceId, int startPage, int pageSize, String keyword,
			String containedInstance, List<Service> serviceList, boolean hasIpCount) {
