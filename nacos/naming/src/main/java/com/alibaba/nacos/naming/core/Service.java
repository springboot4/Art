 ...
 * <p>
 * We introduce a 'service --> cluster --> instance' model, in which service stores a list
 * of clusters, which contain a list of instances.
 * <p>
 * This class inherits from Service in API module and stores some fields that do not have
 * to expose to client.
 *
 * @author nkorange
 */
public class Service extends com.alibaba.nacos.api.naming.pojo.Service implements Record, RecordListener<Instances> {
	...
	/**
	 * IP will be deleted if it has not send beat for some time, default timeout is 30
	 * seconds.
	 */
	...
	public synchronized void recalculateChecksum() {
		...
		try {
			...
			try {
				...
				result = new BigInteger(1, md5.digest((ipsString.toString()).getBytes(Charset.forName("UTF-8"))))
						.toString(16);
			}
			catch (Exception e) {
