...
public class TcpSuperSenseProcessor implements HealthCheckProcessor, Runnable {
	...
	private static ScheduledExecutorService TCP_CHECK_EXECUTOR = new ScheduledThreadPoolExecutor(1,
			new ThreadFactory() {
				@Override
		...
		for (Instance ip : ips) {
			...
			if (!ip.markChecking()) {
				SRV_LOG.warn("tcp check started before last one finished, service: "
						+ task.getCluster().getService().getName() + ":" + task.getCluster().getName() + ":"
						+ ip.getIp() + ":" + ip.getPort());
	...
	public class PostProcessor implements Runnable {
		...
		public void run() {
			...
			try {
				if (!beat.isHealthy()) {
					// invalid beat means this server is no longer responsible for the
					// current service
					key.cancel();
					key.channel().close();
