...
public class NamingProxy {
			...
			HttpClient.asyncHttpPutLarge(
					...
					headers, JSON.toJSONBytes(checksumMap), new AsyncCompletionHandler() {
						...
						public void onThrowable(Throwable t) {
							Loggers.DISTRO
									.error("failed to req API:" + "http://" + server + RunningConfig.getContextPath()
	...
	public static boolean syncData(byte[] data, String curServer) throws Exception {
		try {
			...
			HttpClient.HttpResult result = HttpClient.httpPutLarge("http://" + curServer
					+ RunningConfig.getContextPath() + UtilsAndCommons.NACOS_NAMING_CONTEXT + DATA_ON_SYNC_URL, headers,
					data);
	...
	public static String reqAPI(String api, Map<String, String> params, String curServer) throws Exception {
		try {
			...
			throw new IOException("failed to req API:" + "http://" + curServer + api + ". code:" + result.code
					+ " msg: " + result.content);
		}
		catch (Exception e) {
			Loggers.SRV_LOG.warn("NamingProxy", e);
