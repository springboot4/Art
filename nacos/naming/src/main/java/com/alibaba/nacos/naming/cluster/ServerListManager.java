...
public class ServerListManager {
			...
			try {
				...
				if (!contains(NetUtils.localServer())) {
					Loggers.SRV_LOG.error("local ip is not in serverlist, ip: {}, serverlist: {}",
							NetUtils.localServer(), allServers);
					return;
				}
				...
				GlobalExecutor.registerServerStatusReporter(this,
						switchDomain.getServerStatusSynchronizationPeriodMillis());
			}
	...
	private void checkDistroHeartbeat() {
		...
		for (Server server : servers) {
			...
			server.setAdWeight(
					switchDomain.getAdWeight(server.getKey()) == null ? 0 : switchDomain.getAdWeight(server.getKey()));
		...
		if (autoDisabledHealthCheck && curRatio > switchDomain.getDistroThreshold()
		...
		if (!CollectionUtils.isEqualCollection(healthyServers, newHealthyList)) {
			...
			if (switchDomain.isHealthCheckEnabled()) {
				Loggers.SRV_LOG.info(
						"[NACOS-DISTRO] healthy server list changed, "
								+ "disable health check for {} ms from now on, old: {}, new: {}",
						STABLE_PERIOD, healthyServers, newHealthyList);
