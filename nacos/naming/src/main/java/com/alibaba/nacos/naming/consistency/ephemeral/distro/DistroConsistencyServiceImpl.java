 ...
 * <p>
 * At mean time every Nacos server receives data sync of other Nacos server, so every
 * Nacos server will eventually have a complete set of data.
 *
 * @author nkorange
 * @since 1.0.0
 */
...
public class DistroConsistencyServiceImpl implements EphemeralConsistencyService {
	...
	public void onReceiveChecksums(Map<String, String> checksumMap, String server) {
		...
		try {
			...
			if (Loggers.DISTRO.isDebugEnabled()) {
				Loggers.DISTRO.info("to remove keys: {}, to update keys: {}, source: {}", toRemoveKeys, toUpdateKeys,
						server);
			}
				...
				try {
					...
					for (RecordListener listener : listeners.get(datumKey)) {
						...
						try {
							...
							Loggers.DISTRO.error("[NACOS-DISTRO] error while notifying listener of key: {}", datumKey,
									e);
						}
					}
					...
					if (Loggers.DISTRO.isDebugEnabled()) {
						Loggers.DISTRO.debug(
								"[NACOS-DISTRO] datum change notified, key: {}, listener count: {}, action: {}",
