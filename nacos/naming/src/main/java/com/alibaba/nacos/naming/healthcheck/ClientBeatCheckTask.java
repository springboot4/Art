...
public class ClientBeatCheckTask implements Runnable {
					...
					if (!instance.isMarked()) {
						if (instance.isHealthy()) {
							...
							Loggers.EVT_LOG.info(
									...
									UtilsAndCommons.LOCALHOST_SITE, instance.getInstanceHeartBeatTimeOut(),
	...
	private void deleteIP(Instance instance) {
		...
		try {
				...
				public Object onCompleted(Response response) throws Exception {
					if (response.getStatusCode() != HttpURLConnection.HTTP_OK) {
						Loggers.SRV_LOG.error(
								"[IP-DEAD] failed to delete ip automatically, ip: {}, caused {}, resp code: {}",
			...
			Loggers.SRV_LOG.error("[IP-DEAD] failed to delete ip automatically, ip: {}, error: {}", instance.toJSON(),
					e);
		}
	}
