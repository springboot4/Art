...
/**
 * A lowest precedence {@link EnvironmentPostProcessor} implementation to append Nacos
 * default {@link PropertySource} with lowest order in {@link Environment}
 *
...
public class NacosDefaultPropertySourceEnvironmentPostProcessor implements EnvironmentPostProcessor, Ordered {
	...
	private void processPropertySource(ConfigurableEnvironment environment, ResourceLoader resourceLoader) {
		...
		try {
			...
			// append nacosDefaultPropertySource as last one in order to be overrided by
			// higher order
