...
public class DiskCache {
	...
	public static void write(ServiceInfo dom, String dir) {
		...
		try {
			...
			if (!file.exists()) {
				// add another !file.exists() to avoid conflicted creating-new-file from
				// multi-instances
	...
	public static Map<String, ServiceInfo> read(String cacheDir) {
		...
		try {
			...
			for (File file : files) {
				...
				if (!(fileName.endsWith(Constants.SERVICE_INFO_SPLITER + "meta")
						|| fileName.endsWith(Constants.SERVICE_INFO_SPLITER + "special-url"))) {
					...
					if (newFormat != null && !StringUtils.isEmpty(newFormat.getName())
							&& !CollectionUtils.isEmpty(newFormat.getHosts())) {
